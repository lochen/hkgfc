...
HKGCore.prototype = {
	init : function (data) {
		...
		console.log('HKGCore.init()', data, self.hash);
		...
		$("#switch_server").find('a').click(function (e) {
			//var _fserver=$(this).attr('data-server');
				...
				$.getScript('js/ping.js',function(){
					$("#switch_server").find('a[class!=ping]').each(function(i,a){	
						...
						pingForum(_furl,null,function(presult){
							...
							if(presult.seconds>=30)presult.seconds='>'+presult.seconds;
	...
	buildView : function (id, data) {
		...
		if (data.list.length != 0) {
			...
			$.each(data.list, function (i, d) {
				...
				//content+='<div class="quote"><a href="#" class="btn"><i class="icon-retweet"></i></a></div>';
			...
			$(t_table).find('.content').find('.img-zoom').each(function (i, el) {
				$(el).click(function () {
					...
					if(gallery_url.length!=0){
						var index=$.inArray(src, gallery_url);
							if(index!=-1){
								...
								//footer+='<span class="hide index">'+index+'</span>';
					...
					$(img).load(function () {
						...
						//$("#" + imgid).find('img').attr('src', src);
						fullscreen($("#" + imgid),{offset:10,callback: function(){
							...
							$("#" + imgid).find('.modal-body').css({height:h,'max-height':'none'});
							$("#preload_img").removeClass('hide').appendTo($("#" + imgid).find(".tcenter"));
							}
						});
						//$("#" + imgid).find('img.photo').show();
			...
			if(_furl.match(/youtube/)){
				var _uid=_furl.replace(/v=(.+)/,"$1");
				_furl="http://www.youtube.com/embed/"+_uid;
				alert(_furl);
			}
			var html='<div class="tcenter" style="height:100%;"><img src="img/loading.gif" class="loading"><iframe id="iframe" src="'+_furl+'" width="100%" height="100%" frameborder="0" hspace="0" class="hide"></iframe></div>';
			var modal=bootbox.modal(html,'External Page',{id: 'iframe_box', width: $(window).width()-20, height:$(window).height()-20});
				
				
				$("#iframe").load(function(){
					$("#iframe_box").find('.modal-body').css({'max-height':'none'});
					console.log($("#iframe_box").find('.modal-body').height());
					...
					$("#iframe").css({width: $("#iframe_box").find('.tcenter').width()+5,'height':h}).fadeIn();
					$("#iframe_box").find('.loading').hide();
				}).error(function(){
					$("#iframe_box").find('.tcenter').html('Load error');
					$("#iframe_box").find('.loading').hide();
				});
				//$('.bootbox').css({}).width(1024);
				
				/*$('.bootbox').css({
						position:'absolute',
						top:$('.bootbox').position().top,
						left: $('.bootbox').position().left
					}).removeClass('fade in');*/
					/*fullscreen($('.bootbox'),{
						speed: 500,
						offset: 10
				...
				var m = buildModal({
							id : 'link_iframe',
							header : 'LINK',
							body : '<div class="tcenter"><img src="img/loading.gif" class="loading"><iframe id="iframe" src="'+$(this).attr('href')+'" width="100%" height="100%" frameborder="0" hspace="0" class=""></iframe></div>'
						});
				$(m).modal();
				console.log($('#link_iframe').width(),$('#link_iframe').height());
				setTimeout(function(){
				
				console.log($('#link_iframe').position());
				$('#link_iframe').css({
						position:'absolute',
						top:$('#link_iframe').position().top,
						left: $('#link_iframe').position().left
					}).removeClass('fade in');
					/*$('#link_iframe').css({
						position:'absolute',
						//top:10,
					...
					fullscreen($('#link_iframe'),{
						speed: 500,
						offset: 10,
						callback: function(){
							$("#iframe").load(function(){
								//$('html,body').height('100%');
								$('#link_iframe').find('img.loading').hide();
								var h=$('#link_iframe').outerHeight()-$('#link_iframe').find('.modal-header').outerHeight()-$('#link_iframe').find('.modal-footer').outerHeight()-30-4;
								//$('#link_iframe').find('.modal-body').css({'height':h,'max-height':'none'});
								//console.log('modal-body height',$('#link_iframe').outerHeight(),$('#link_iframe').find('.modal-header').outerHeight(),$('#link_iframe').find('.modal-body').css('padding'),$('#link_iframe').find('.modal-footer').outerHeight(),h);
								//$("#iframe").show();
							});
						}
					});
	...
	convertContent : function (content) {
		...
		if (link.length != 0) {
			...
			$.each(link, function (i, d) {
				...
				//if(!isimg) url += 'NOT IMG';
				//if(!isimg)url += '<a href="'+d.href+'" class="link-zoom"><span class="icon-fullscreen vmiddle"></span></a>';
...
//		console.log('storage settings', this.storage.settings);
		//if(this.storage.settings.type=='db'){
		//	console.log(data);
		//	this.storage.set(["tid","title","content","replies","dateline","read_time"],[parseInt(data.id),data.title,"",0,parseInt(data.time),parseInt(new Date().getTime())],{table:'topic',callback:function(){console.log('query cb');}});
	//	}
		return true;
	},
	...
	loggedin : function(type,callback){
		if(isUndefined(type))var type = 'all';
		var list = [], m_expire = '';
		chrome.cookies.getAll({
			...
			$.each(data, function (i, d) {
				...
				if (d.value != ''){
					list.push({'server':_server,'expire':_expire});
				}
				if(d.domain=='m.hkgolden.com'){
					m_expire=_expire;
					list.push({'server':'m','expire':_expire});
				}
			});
			var loggedin=[];
			if(type=='all'){
				for (var i = 1; i <= 11; i++) {
					...
					loggedin.push({'server':i,'expire':t});
				}
				loggedin.push({'server':'m','expire':m_expire})
			}
			else{
				loggedin=list;
			}
