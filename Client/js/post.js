...
function postBox(option) {
	...
	if (options.mode == 'topic') {
		noty({
			text : 'POST FUNCTION IS NOT AVAILABLE',
			layout : 'center',
			type : 'error',
			timeout : 3000
		});
	...
	if (options.mode == 'topic')
		_body += '<tr><td><input type="text" id="post_topic" name="" value="" placeholder="Topic"></td></tr>';
	...
	setTimeout(function () {
		if ($("#post_topic").length != 0)
			...
			$("#post_content").focusEnd();
		
		// reset window scroll when the modal closed
		$("#post_box").on('hidden', function () {
			$(document.body).css({
				'overflow' : 'auto',
				'overflow-x' : 'auto',
				'overflow-y' : 'auto'
		...
		$('<button class="btn smile"><i class="icon-smile"></i></button>').click(function () {
			iconList();
		}).appendTo($(other));
		...
		$("#bbcode_bar").find('.photo, .url').click(function () {
			...
			$("#popup_text").val(v).select().bind('click', function () {
				$(this).select();
			}).blur(function (e) {
				console.log('blur', e.target);
				$(document).one('click', function (e) {
					console.log('blur doc click', e.target);
					if ($(e.target).attr('tagName') != "button")
						$("#popup_box").hide();
				});
			});
			console.log(code);
		...
		$(document.body).unbind('keydown').bind('keydown', function (e) {
			if (e.altKey) {
				...
				e.originalEvent.keyCode = 0;
				if (cls != "")
					$("#bbcode_bar").find(cls).trigger('click');
			}
		});
		...
		var loggedin = hkgfc.loggedin('single', function (data) {
				...
				$('<span class="label label-warning">Server ' + server + '</span>').appendTo($("#post_box").find('.modal-footer').find('.pull-left'));
			});
		
		$("#post_box").find('.modal-confirm').click(function () {
			if ($("#post_topic").length != 0 && $("#post_topic").val() == '') {
				noty({
					text : 'Topic can not empty',
					layout : 'center',
					type : 'error',
					timeout : 3000
				});
