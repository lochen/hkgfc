...
HKGCore.prototype = {
	init : function (data) {
		...
		//switch server
		$("#switch_server").find('a').click(function () {
	...
	topic : function (data) {
		...
		self.xhr = $.ajax({
				...
				success : function (response, status) {
					...
					//is logined
					if ($(html).find("a[href*='ProfilePage.aspx']").length != 0) {
	...
	buildView : function (id, data) {
		...
		if (data.list.length != 0) {
			...
			$.each(data.list, function (i, d) {
				...
				isauthor = '';
				if (i == 0 && data.page == 1) {
					score = '<div class="btn-group"><button class="btn"><img src="faces/ThumbUp.gif">' + data.score[0] + '</a></button><button class="btn"><img src="faces/ThumbDown.gif">' + data.score[1] + '</a></button></div>';
				}
			...
			$(t_table).find('.content').find('.img-zoom').each(function (i, el) {
				$(el).click(function () {
					...
					var m = buildModal({
							...
							body : '<div class="tcenter"><img src="img/loading.gif"></div>'
						});
					$(m).modal();
	...
	convertContent : function (content) {
		...
		if (link.length != 0) {
			$.each(link, function (i, d) {
				var attr = (d.href != '#') ? ' target="_blank"' : '';
				
				console.log(d);
				if (!isUndefined(d.href) && d.href.match(/(png|jpg|gif)/i)) {
